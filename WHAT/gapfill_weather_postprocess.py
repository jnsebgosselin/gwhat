...
def load_err_file(fname): #================================== Load err. File ==
    
    with open(fname) as f:
        reader = list(csv.reader(f, delimiter='\t'))
    
    #---- Finds First Row of Data ----
    ...
    varTypes = ['continuous'] * (len(varNames))
    ...
    for i, var in enumerate(varNames):
        ...
        Yp.append(DATA[indx, 5].astype(float))
        Ym.append(DATA[indx, 6].astype(float))
        ...
        for date in range(len(y)):
            t[date] = (xldate_from_date_tuple((y[date], m[date], d[date]), 0) -
        ...
        # If the proportion of zeros in the data series is higher than 25%, 
        # the data type is set as an event-based weather variable. Otherwise, 
        # default value is kept and variable is considered to be continuous
        # in time.
        #
        # The precipitation (solid, liquid or total) is a good example of
        # an event-based variable, while air temperature (min, max or mean) is
        # a good example of a continuous variable.
      
        pc0 = len(np.where(Ym[i] == 0)[0]) / float(len(Ym[i]))
        if pc0 > 0.25:
            varTypes[i] = 'event-based'
        
    return(Yp, Ym, Time, Date, varNames, varTypes)
        
        
def plot_est_err(Yp, Ym, name): #================== Est. Errors Scatter Plot ==
    
    Ypre = np.array(Yp).astype(float)
    Ymes = np.array(Ym).astype(float)    
    Ymax = int(np.max(Ymes) / 10) * 10
    Ymin = int(np.min(Ymes) / 10) * 10
    ...
    hbl, = ax0.plot([Ymin, Ymax], [Ymin, Ymax], 'k', lw=dlw, dashes=[dl, ds],
                    dash_capstyle='butt')
    ...
    if name in ['Max Temp (deg C)', 'Mean Temp (deg C)', 'Min Temp (deg C)']:
        units = u'°C'
    elif name in ['Total Precip (mm)']:
        units = 'mm'
    else:
        units = ''
    ...
    if name == 'Max Temp (deg C)':
        var = u'Daily Max Temperature (°C)'
    elif name == 'Mean Temp (deg C)':
        var = u'Daily Mean Temperature (°C)'
    elif name == 'Min Temp (deg C)':
        var = u'Daily Min Temperature (°C)'
    elif name == 'Total Precip (mm)':
    ...
    ax0.legend([hscat, hbl], ['Daily Weather Data', '1:1'], loc='upper left',
               numpoints=1, frameon=False, fontsize=16)
...
def plot_gamma_dist(Ymes, Ypre): #=========================== Plot Gamma PDF ==
    ...
    x = np.arange(0.1, Ymax, 0.1)
    ax0.plot(x, stats.gamma.pdf(x, alpha, loc=loc, scale=beta), '--r', lw=2,
             alpha=0.85, color=c2, label='Gamma PDF (estimated)')
    ...
    ax0.yaxis.set_ticks_position('left')
    ax0.tick_params(axis='both', direction='out', labelsize=14)
    ax0.tick_params(axis='both', which='minor', direction='out', labelsize=14)
    ...
    fig.savefig('precip_PDF.pdf') # A canvas.draw() is included with this.
    return canvas
...
if __name__ == '__main__': #=========================================== Main ==
    ...
    Yp, Ym, Time, Date, varNames, varTypes = load_err_file(fname)
